...
for j=1:VMAT_PLN_INFO.Total_CPs-1
   ...
   if VMAT_PLN_INFO.CP_info{j,1} ~= VMAT_PLN_INFO.CP_info{j+1,1}
       ...
       max_MU_interval = 2.0341/Max_gantry_speed*Max_dose_rate/60;
...
for jj=j+1:VMAT_PLN_INFO.Total_CPs-1
   ...
   if VMAT_PLN_INFO.CP_info{jj,1} ~= VMAT_PLN_INFO.CP_info{jj+1,1}
       ...
       if delta_MU <= max_MU_interval
           ...
           time_interval(jj) = 2.0341/Max_gantry_speed;
...
MLC_speed_mat_1 = zeros(size(VMAT_PLN_INFO.CP_info{1, 3},1),VMAT_PLN_INFO.Total_CPs-1); % construct the MLC speed matrix (num of MLC leaves x num of control points)
...
MLC_speed_mat_1(:,flag) = 0;
VMAT_PLN_INFO.MLC_speed_std = std(MLC_speed_mat_1,0,2);


% initialize the Z distribution
f = 0:0.01:2;
Z = zeros(size(VMAT_PLN_INFO.MLC_speed_std,1),length(f));
X = abs(MLC_speed_mat_1); % absolute value of leaf speed
...
VMAT_PLN_INFO.MLC_speed_mat_1 = MLC_speed_mat_1; % a matrix (160 leaf x 219 control points)
