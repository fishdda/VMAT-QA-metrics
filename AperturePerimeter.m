function Aperture_Perimeter = AperturePerimeter(VMAT_PLN_INFO,BLD_type)
%
...
for k= 1:length(seg_perimeter)
    ...
    %             '.BeamLimitingDevicePositionSequence.Item_1.LeafJawPositions;'])
        ...
        % determine the first leaf position
        ...
        elseif rem(start_point,1) ~= 0 && rem(end_point,1) == 0
            ...
            seg_perimeter(k) = seg_perimeter(k) + (mlc_leaf(start_point,2)-mlc_leaf(start_point,1))*first_width/100;
            for j = start_point+1:end_point
                 seg_perimeter(k) =  seg_perimeter(k) + (mlc_leaf(j,2)-mlc_leaf(j,1))*mlc_width/100;   
            end
        ...
        elseif rem(start_point,1) == 0 && rem(end_point,1) ~= 0
            ...
            disp(end_point+1)
            seg_perimeter(k) = seg_perimeter(k) + (mlc_leaf(end_point+1,2)-mlc_leaf(end_point+1,1))*last_width/100;
            for j = start_point:end_point
                 seg_perimeter(k) =  seg_perimeter(k) + (mlc_leaf(j,2)-mlc_leaf(j,1))*mlc_width/100;   
            end
        ...
        elseif rem(start_point,1) ~= 0 && rem(end_point,1) ~= 0
            ...
            disp(seg_perimeter(k));
            
        end

VMAT_PLN_INFO.CP_info{k,6} = seg_perimeter(k);
end
end
